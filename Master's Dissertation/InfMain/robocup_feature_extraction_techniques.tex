...
This chapter describes the main feature extraction algorithms that are to be compared in order to determine the best algorithm to be implemented on the Nao humanoid robot for Robocup. These algorithms can be used as the feature extraction module in \figref{fig:system}. SBRISK and SU-BRISK are two types of feature extraction algorithms developed by \citet{Leutenegger2011}. SBRISK will be referred to as BRISK0 and SU-BRISK will be referred to as BRISK0 - U-BRISK for visualisation purposes. Variations of the original BRISK implementation \citep{Leutenegger2011} have been utilised and include BRISK0, BRISK0 - SURF2D and BRISK0 - U-BRISK. The BRISK source code used for this project has been taken from the open-source package available online \citep{briskCode}.\\

1D SURF has already been developed and implemented on a Nao robot by the \textit{rUNSWift} Robocup team \citep{Anderson} and is used in this thesis as a means of comparison with the BRISK-based algorithms. The 1D SURF source code has been provided by the \textit{rUNSWift} team. The detector and descriptor for each feature extraction algorithm are displayed in \tabref{tab:detector_descriptor}.\\
