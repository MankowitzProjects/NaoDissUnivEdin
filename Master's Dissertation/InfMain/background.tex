...
\begin{figure}[h!] 
  \centering
    \includegraphics[width=0.4\textwidth]{../Drawings/methods/SURF2D_BoxFilters_only.jpg}
    \caption{The Box filters used to perform the 2D convolutions \citep{Bay2008}}
    \label{fig:boxFilters}
\end{figure}

Therefore, by utilising box filters, the determinant of the Hessian is computed as shown in \eqnref{eqn:approxHessian}. The variables $D_{xx}$, $D_{xy}$ and $D_{yy}$ represent the box filter convolutions in the $x$, $xy$ and $y$ directions respectively at a point $\textbf{x} = (x,y)$ in the image $I$.\\ 
...
The determinant of the Hessian at a point $\textbf{x} = (x,y)$ in the image $I$ is referred to as the \textit{blob response} at a scale $\sigma$ \citep{Bay2008}. The \textit{blob response} is computed at every location in the image in order to create a \textit{blob response map}. A \textit{blob response map} exists for every considered scale and these maps are used to detect local maxima in the images.\\

The different scales collectively define the scale space. The scale space can be seen as a function that is used to find extrema across a set of predefined scales. The scale space is important as it ensures that interest points detected in this space are scale invariant.\\
...
\begin{figure}[h!] 
  \centering
    ...
    \caption{The up-scaling of the filters is used in 2D SURF rather than the down-scaling of the image \citep{Evans2009}}
    \label{fig:scaleSpace}
\end{figure}

The scale space is divided into octaves. An octave consists of a set of \textit{blob response maps} each resulting from a convolution of the image with a box filter of a specific size. The \textit{blob response map} corresponding to the lowest level (i.e. smallest scale) of the scale space is constructed using a $9 \times 9$ box filter. This corresponds to a scale value of $1.2$. As the size of the filters used for convolution increase, so too does the scale. During the convolution procedure, the filter is centered on each $(x,y)$ location in the image and each lobe in the filter has a specific width represented by $l_0$. In order to increase the size of the filter (I.e. move up a level in scale space), the filter lobe width, $l_0$, is increased by $2$ pixels. This ensures that the center pixel is still found in the center of the filter. Thus, the first filter is $9 \times 9$ and the next successive filter is $15 \times 15$. \\
...
As mentioned previously, the convolution of the box filters of various sizes produces \textit{blob response maps} at various scales. The next step involves using the \textit{blob response maps} to detect interest points at these different scales. The first step in detecting interest points is removing pixels whose \textit{blob responses} are below a certain threshold. Once the relevant pixels have been thresholded,  a non-maximal suppression is performed in a $3 \times 3 \times 3$ neighborhood surrounding the current pixel. Thus each pixel is compared to its $26$ neighbors in scale space \citep{Evans2009}. This includes its $8$ adjacent neighbors in image space as well as its $9$ neighbors in the scale above and scale below the current image space respectively as shown in \figref{fig:imageSpace}. It should be noted that the largest scale and smallest scale for each octave have no scale space layers above or below respectively. Therefore these \textit{blob response maps} are only used for comparison for the internal scale space layers. \\

\begin{figure}[h!] 
  \centering
    \includegraphics[width=0.5\textwidth]{../Drawings/methods/SURF2D_Nonmaximal_suppression.pdf}
    \caption{The non-maximal suppression performed on each point and its $26$ surrounding neighbors \citep{Lowe2004}}
...
\subsection{BRISK}
\label{sec:brisk}
A new method has been developed by \citet{Leutenegger2011} that yields computational performance bettering SURF by an order of magnitude in various domains. This method is called Binary Robust Invariant Scalable Keypoints (BRISK) \citep{Leutenegger2011}. This method utilises a technique called Adapative and Generic Corner Detection Based on the Accelerated Segment Test (AGAST) in order to compute a score for a pixel in location $(x,y)$ in image $I$ \citep{Mair2010}. AGAST is largely based on the Features from Accelerated Segment Test (FAST) \citep{Rosten2006} method but incorporates some improvements such as adaptive tree switching which will be briefly discussed. If the pixel's score is above a predefined threshold, then it is detected as an interest point. A binary feature descriptor of length $512$ is then generated for each interest point as a result of some simple brightness comparison tests. Interest points are then matched in different images by computing the Hamming distance between the feature descriptors. \\

Due to the computational performance, this method is more suitable than SURF for a resource constrained system. One of the main reasons for this performance is due to the FAST-based interest point detector. This detector computes a score called the FAST score for each pixel and uses this as one of the criterion to identify interest points.\\

\subsubsection{FAST Score}
\label{sec:fastScore}

The FAST score, denoted as $s$, is the first criterion that is used in BRISK in order to identify interest points \citep{Rosten2006}. The FAST score is computed by initially choosing a set of $16$ pixels that form a circle centered on the current pixel $p$. $p$ is the potential interest point that is being evaluated, as shown in \figref{fig:fastScore}. The circle has a Bresenham radius of $3.4$ pixels \citep{Mair2010}. For the pixel $p$ to be considered an interest point, there has to be $n$ (in this case $9$) contiguous pixels from the circle of $16$ pixels that are brighter than the current pixel intensity $I_p + t$, where $t$ is a predefined threshold, or are darker than $I_p - t$. A larger value of $t$ will reduce the amount of interest points that will be detected. However, these detected points will be strong, salient interest points. \\

\begin{figure}[h!] 
  \centering
    \includegraphics[width=0.8\textwidth]{../Drawings/methods/FASTScoreCalculation.pdf}
    \caption{The calculation of the FAST score for BRISK \citep{Rosten2006}}
    \label{fig:fastScore}
\end{figure}
...
\subsubsection{Detector}
\label{briskDetect}
BRISK's scale space has a different structure to that of SURF. In this case, the scale space consists of \textit{n} octaves \textit{$c_i$} and \textit{n} intra-octaves \textit{$d_i$} \citep{Leutenegger2011}. The typical number of octaves chosen for BRISK is $n=4$. The intra-octaves are placed between the octaves. A typical ordering would be $c_0, d_0, c_1, d_1, c_2...$. The first octave corresponding to the original image is called $c_0$ and the first intra-octave $d_0$ is generated by down-sampling the original image by a factor of $1.5$. Thereafter subsequent octaves are created by half-sampling the image by a factor of two. This creates an image pyramid where the lowest layer is the original image and the higher layers are down-sampled versions of the original image.\\
...
\begin{figure}[h!] 
  \centering
    ...
    \caption{The sampling pattern for an interest point. Each blue circle represents a sample and the red circle size is proportional to the amount of smoothing performed on each respective sample \citep{Leutenegger2011}}
...
This gradient is then used to find the angle with which to rotate the sampling pattern around the interest point in order to achive rotation invariance. The angle is computed as shown in \eqnref{eqn:angle}. As in SURF, for the purpose of Robocup, assigning an orientation might not be necessary since the Nao robot is not subject to large rotations whilst playing football. This would also make the method more computationally efficient.\\
...
In summary, SURF 2D is a robust feature extraction algorithm that is more computationally efficient than SIFT \cite{Bay2008}. It has a lower $64$ dimensional descriptor and performs efficient computations using integral images and box filters. However, BRISK achieves even better computational performance whilst still maintaining comparative overall performance with SURF \cite{Leutenegger2011}. This method is therefore more suitable for a resource constrained Nao robot. Variations of this method such as removing rotation invariance will speed up the method and may be suitable for the Robocup scenario.\\
