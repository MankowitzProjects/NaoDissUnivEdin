...
In total $600$ overlapping pairs of images and $600$ non-overlapping pairs of images were generated respectively. Utilising this data, the optimal thresholds for each of the four feature extraction algorithms were calculated using the optimal detection parameters algorithm detailed in \secref{sec:optimalParameters}. This generated the MTS and CPS parameter values and these are displayed in \tabref{tab:dataset2OptimalThresholds}.\\
...
Utilising the optimal parameters, the feature extraction algorithms were tested on each of the overlapping and non-overlapping image pairs. ROC curves were then generated and are displayed in \figref{fig:compareKnnAdditional} and \figref{fig:compareKnnConsistentAdditional} respectively. \figref{fig:compareKnnAdditional} shows the ROC curves that were generated when using the MPS setting. \figref{fig:compareKnnConsistentAdditional} has been generated using the CPS setting. As can be seen in the figures, the ROC curves exhibit good matching performance for each of the algorithms. The statistics quantifying the performance are shown in \tabref{tab:rocTimingAdditional}. As can be seen in the table, BRISK0 has the best AUV value of $98.037\%$. However, timing is also a big concern and thus the best overall feature extraction algorithm is again the BRISK0 - U-BRISK combination. It performs a match between two images in the fastest time of $13 ms$ in the worst case, and has performance comparable to BRISK0.\\
...
\begin{figure}[h!]
\begin{minipage}[b]{0.5\linewidth}
\includegraphics[scale=0.5]{../Drawings/dataset2_ROC_General_KNN.jpg}
\caption{A comparison of the ROC curves for BRISK0, BRISK and BRISK0-SURF2D using Hamming distance for matching on the additional dataset}
\label{fig:compareKnnAdditional}
\end{minipage}
\hspace{0.5cm}
\begin{minipage}[b]{0.5\linewidth}
\includegraphics[scale=0.5]{../Drawings/dataset2_ROC_General_KNN_Consistent.jpg}
\caption{A comparison of the ROC curves for BRISK0, BRISK and BRISK0-SURF2D using Hamming distance for matching and consistent thresholds on the additional dataset}
\label{fig:compareKnnConsistentAdditional}
\end{minipage}
\end{figure}
