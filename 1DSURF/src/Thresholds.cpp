...
int main(int argc, char *argv[])
{
	...
	for (int ss = 5;ss<=6;ss++)
	{
		...
		string tempDir = to_string<int>(ss);
		...
		string file = "data/Thresholds/thresholds_SURF1D_19072012_1151";
		//*****************************************
		file.append(".txt");
		cout<<file<<endl;
		...
		for (int tt = 1; tt<=20;tt++)//20
		{
			...
			for (int ii = 1;ii<=jpegCounter;ii++)
			{
				...
				for (int jj = 1; jj<ii;jj++)//jpegCounter1
				{
					...
					writeFile <<tempDir<<", "<<THRES<<", "<<name1<<", "<<name2<<", "<<image1.ipts.size()<<", "<<image2.ipts.size()<<", "<<result<<", "<<0<<", "<<Ipoint::totalNumMatches<<", "<<Ipoint::numValidMatches<<","<<Ipoint::totalNumMatches -Ipoint::numValidMatches<<", "<<0<<", "<<Ipoint::detectionTime<<", "<<Ipoint::extractionTime<<", "<<Ipoint::matchingTime<<", "<<Ipoint::verificationTime<<", "<<Ipoint::overallTime<<"\n";
