%Matching with the old pics and time included
%Data analysis for Master's Thesis
clc;
load 'oldMatching.mat'

matchingData = matchingData_Jun_04_160115;
...
%-----------------------------------------
%Times
%10 - Detection time (ms)
%11 - Extraction Time (ms)
%12 - Matching Time (us)
%13 - Overall Time (ms)
disp('------------------Times-------------------')
%Mean detection time
meanDetectionTime = mean(matchingData(:,10))

%Mean extraction time
meanExtractionTime = mean(matchingData(:,11))

%Mean Matching time
meanMatchingTime  = mean(matchingData(:,12));
meanMatchingTime = meanMatchingTime/1000

%Mean overall time
meanOverallTime = mean(matchingData(:,13))
