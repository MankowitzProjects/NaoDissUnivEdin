...
{
  // convert the 8 bit depth image to single channel 32f
  IplImage *img = getGray(source);
  //Create the integral image
  IplImage *int_img = cvCreateImage(cvGetSize(img), IPL_DEPTH_32F, 1);//IPL_DEPTH_32F

  // set up variables for data access
  int height = img->height;
  int width = img->width;
  int step = img->widthStep/sizeof(float);//sizeof(float)
