...
void FastHessian::getIpoints()
{
	...
	for (int o = 0; o < octaves; ++o) for (int i = 0; i <= 1; ++i)
	{
		...
		//Calculate the single row of pixels as a vertical average of 40 pixels
...
void FastHessian::buildResponseMap()
{
	...
	if (octaves >= 2)
	{
		responseMap.push_back(new ResponseLayer(w/2, h, s*2, 39));//h/2
		responseMap.push_back(new ResponseLayer(w/2, h, s*2, 51));//h/2
	}

	if (octaves >= 3)
	{
		responseMap.push_back(new ResponseLayer(w/4, h, s*4, 75));//h/4
		responseMap.push_back(new ResponseLayer(w/4, h, s*4, 99));//h/4
	}
	if (octaves >= 4)
	{
		responseMap.push_back(new ResponseLayer(w/8, h, s*8, 147));//h/8
		responseMap.push_back(new ResponseLayer(w/8, h, s*8, 195));//h/8
	}

	if (octaves >= 5)
	{
		responseMap.push_back(new ResponseLayer(w/16, h, s*16, 291));//h/16
		responseMap.push_back(new ResponseLayer(w/16, h, s*16, 387));//h/16
	}

	// Extract responses from the image
	for (unsigned int i = 0; i < responseMap.size(); ++i)
	{
		//Here is where we calculate an approximation of the Hessian
...
//Top, middle and bottom response layers
//Fix: We only want to check along a single layer, not all 3.
int FastHessian::isExtremum(int r, int c, ResponseLayer *t, ResponseLayer *m, ResponseLayer *b)
{
	...
	//			if (
	//					//MC: This will have to be removed or an additional if statement is added
